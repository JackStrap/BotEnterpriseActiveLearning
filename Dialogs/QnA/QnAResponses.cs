...
{
	public class QnAResponses : TemplateManager
	{
		/// <summary>
		...
		{
			List<QueryResult> filteredQnaSearchResult = new List<QueryResult>();
			...
			float topAnswerScore = qnaSearchResults[0].Score * 100;
			...
			if ((topAnswerScore > MinimumScoreForLowScoreVariation) && (topAnswerScore < MaximumScoreForLowScoreVariation))
			{
				...
				for (int i = 1; i < qnaSearchResults.Count; i++)
				{
		...
		{
			string uri = host + "/knowledgebases/" + kbId + "/train/";
